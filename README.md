...
| [The Lamp](the-lamp.md) | A starter kit for persistent memory. A model has no memory between sessions, so the state lives in files you re-inject at the top of each one. Includes the folder, the starting contents of every file, and the setup prompt. |
